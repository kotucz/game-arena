...
fun ContactsBoardState.handleAddHint(
    ...
    if (!isOwnedBy(player, playerContact)) return ActionExecutionResult.Failure("Player does not own the selected contact")
...
fun ContactsBoardState.handleStandardConnect(
    ...
    if (!isOwnedBy(player, playerContact)) return ActionExecutionResult.Failure("Player does not own the selected contact")
    if (!isOwnedByAnotherPlayer(player, otherContact)) return ActionExecutionResult.Failure("Selected opposing contact is not owned by another player")
...
fun ContactsBoardState.handleMultiConnect(
    ...
    // basic validation
    if (!actionType.matches(1, otherContacts.size)) return ActionExecutionResult.Failure("Invalid number of selected contacts")
    if (!isOwnedBy(player, playerContact)) return ActionExecutionResult.Failure("Player does not own the selected contact")
    if (playerContact.type == ContactsBoardState.ContactType.Red) return ActionExecutionResult.Failure("Red contacts cannot be connected")
    if (otherContacts.any { !isOwnedByAnotherPlayer(player, it) }) return ActionExecutionResult.Failure("Selected opposing contact is not owned by another player")
...
fun ContactsBoardState.handleMyDoubleConnect(
    ...
    if (playerContacts.any { !isOwnedBy(player, it) }) return ActionExecutionResult.Failure("Player does not own the selected contact")
    if (!isOwnedByAnotherPlayer(player, otherContact)) return ActionExecutionResult.Failure("Selected opposing contact is not owned by another player")
...
fun ContactsBoardState.handleSoloConnectRest(
    ...
    if (playerContacts.map { it.matchKey }.toSet().size != 1) return ActionExecutionResult.Failure("Selected contacts must have the same number or all yellow")
    if (playerContacts.any { !isOwnedBy(player, it) || isSolved(it) }) return ActionExecutionResult.Failure("Selected contact must be an unsolved contact owned by the player")
    val matchKey = playerContacts.first().matchKey
    val remainingNotOwnedContacts = pool.filter { it.matchKey == matchKey && !isOwnedBy(player, it) && !isSolved(it) }.toSet()
    if (remainingNotOwnedContacts.isNotEmpty()) return ActionExecutionResult.Failure("Some other player still has that contact")
    val remainingOwnedContacts = pool.filter { it.matchKey == matchKey && isOwnedBy(player, it) && !isSolved(it) }.toSet()
...
fun ContactsBoardState.handleFinishReds(
    ...
    if (playerContacts.any { !isOwnedBy(player, it) || isSolved(it) }) return ActionExecutionResult.Failure("Selected contact must be an unsolved contact owned by the player")
    val unsolvedPlayerContacts = playerRacks(player).flatMap { rack -> rackContacts(rack) }.filter { !isSolved(it) }.toSet()
    if (unsolvedPlayerContacts.any { it.type != ContactsBoardState.ContactType.Red }) return ActionExecutionResult.Failure("All other contacts must be solved")
